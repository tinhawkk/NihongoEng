...
function generateQuestions(words: any[], count = 10, globalDistractorPool: any[] = []) {
    ...
    const loadQuizData = async () => {
      ...
      try {
        ...
        const [extraData, mainData] = await Promise.all([
          vocabularyRepository.loadDeck(randomExtra, "sheet"),
          isSrs ? Promise.resolve([]) : vocabularyRepository.loadDeck(deckId, isVocaSource ? "voca" : "sheet")
        ]);
