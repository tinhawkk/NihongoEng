...
const replacement1 = `export const HomePage = () => {
  ...
  const _handleCreateRoot = async () => {
    ...
    try {
      ...
      alert("ÄÃ£ xáº£y ra lá»—i khi táº¡o danh má»¥c má»›i.");
    } finally {
...
code = code.substring(0, startMatch.index) + replacement1 + code.substring(startMatch.index + startMatch[0].length);

// 3. Find the second chunk: from `];` to `handleQuickAddDeck` end
const endRegex = /];\s*\/\/\s*Save single vocab OR bulk JSON array[\s\S]*?alert\("Lá»—i táº¡o bÃ i nhanh: " \+ err\.message\);\s*\}\s*\};\s*/;
